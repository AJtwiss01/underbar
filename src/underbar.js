(function() {
  ...
  //If n is undefined, return array[last], else if n>array length, return array, else return last n of array.
  ...
  _.each = function(collection, iterator) {
    //check if collection is array or other object.
    if (Array.isArray(collection)) {
      //if array, use for loop to iterate through collection and call iterator on each item.
      for(var i=0,x=collection.length; i<x ; i++) {
        iterator(collection[i], i, collection);
      }
    }
    else {
      //otherwise, use for in loop to call iterator on each key in object.
  ...
  _.map = function(collection, iterator) {
    ...
    _.each(collection, function(value,key,collection) {
      afterAry.push(iterator(value,key,collection));
    });
  ...
  _.invoke = function(collection, functionOrKey, args) {
    ...
    if (typeof functionOrKey === 'string') {
      ...
      return _.map(collection, function(value) {
        return functionOrKey.apply(value,[value,args]);
      });
    }
  };
  ...
  _.zip = function() {
    var collection = _.sortBy(Array.prototype.slice.call(arguments),function(i) {
     return -1 * i.length;
    });

    return _.reduce(collection, function(accum, valAry, indAry) {
      return _.map(accum, function(val, ind) {
        return Array.prototype.concat(accum[ind],valAry[ind]);
      });
    });
  };
  ...
  _.flatten = function(nestedArray, result) {
    var results = [];
    var bracketShredder = function(maybeArray) {
      if(!Array.isArray(maybeArray)) {
        results.push(maybeArray);
      }

      else {
        _.each(maybeArray, function(valAry) {
          bracketShredder(valAry);
        });
      }
    };

    bracketShredder(nestedArray);
